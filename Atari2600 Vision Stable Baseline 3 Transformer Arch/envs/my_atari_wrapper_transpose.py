...
class WarpFrame(gym.ObservationWrapper[np.ndarray, int, np.ndarray]):
    def __init__(self, env: gym.Env, width: int = 84, height: int = 84) -> None:
        ...
        self.stacked_shape = (6, 3, self.height, self.width)
        ...
        self.stacked_obs = np.zeros(self.stacked_shape, dtype=self.observation_space.dtype)
        self.stack_dimension = 0
    ...
    def observation(self, frame: np.ndarray) -> np.ndarray:
        ...
        transposed_frame = frame.transpose((2, 0, 1))
        return transposed_frame

    def reset(
        self, *, seed: int | None = None, options: dict[str, Any] | None = None
    ) -> tuple[WrapperObsType, dict[str, Any]]:
        obs, info = super(WarpFrame, self).reset(seed=seed, options=options)
        self.stacked_obs = np.zeros(self.stacked_shape, dtype=self.observation_space.dtype)
    ...
    def step(
        ...
        obs, buf_rew, terminated, truncated, buf_info = super(WarpFrame, self).step(action)

        self.stacked_obs = np.roll(self.stacked_obs, -1, axis=self.stack_dimension)
        if terminated:
            if "terminal_observation" in buf_info:
                old_terminal = buf_info["terminal_observation"]
                previous_stack = self.stacked_obs[:-1, ...]

                new_terminal = np.concatenate((previous_stack, old_terminal), axis=self.repeat_axis)
                buf_info["terminal_observation"] = new_terminal
                print(buf_info)
                print("I haven't figured out what this is yet.")
                raise NotImplementedError("I haven't figured out what this is yet.")
            self.stacked_obs = np.zeros(self.stacked_shape, dtype=self.observation_space.dtype)

        self.stacked_obs[-1, ...] = obs

        return self.stacked_obs, buf_rew, terminated, truncated, buf_info
...
class AtariWrapper(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    ...
    * Sticky actions: disabled by default
    ...
    #     self, *, seed: int | None = None, options: dict[str, Any] | None = None
    # ) -> tuple[WrapperObsType, dict[str, Any]]:
    #     obs, info = super(AtariWrapper, self).reset(seed=seed, options=options)
    #     print(obs.shape)
    #     print(type(obs))
    ...
    # ) -> tuple[WrapperObsType, SupportsFloat, bool, bool, dict[str, Any]]:
    #     obs, buf_rew, terminated, truncated, buf_info = super(AtariWrapper, self).step(action)
    #     obs = obs.reshape(3, 84, 84)
    #
    #     return obs, buf_rew, terminated, truncated, buf_info
