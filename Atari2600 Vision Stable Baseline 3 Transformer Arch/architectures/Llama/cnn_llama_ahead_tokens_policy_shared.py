...
class LLamaActorCriticNetwork(nn.Module):
    ...
    def forward_actor(self, features: th.Tensor) -> th.Tensor:
        # print("+" * 60)
        output = self.policy_net(inputs_embeds=features)
        # for i in range(0):
        #     new_embd = output.unsqueeze(1)
        #     # print(features.shape, features[:, 1:, ...].shape, new_embd.shape)
        #     features = th.cat((features[:, 1:, ...], new_embd), dim=1)
        #     output = self.policy_net(inputs_embeds=features)
