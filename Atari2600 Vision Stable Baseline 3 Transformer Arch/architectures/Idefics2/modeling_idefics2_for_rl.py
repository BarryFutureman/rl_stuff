...
class Idefics2Dummy(Idefics2PreTrainedModel):
    ...
    def seperate_batch_num_img(
        self,
        batch_size, num_images,
        image_hidden_states: Optional[torch.Tensor],
    ):
        batch_and_num_images, t, vision_hidden_size = image_hidden_states.shape
        reshaped_image_hidden_states = image_hidden_states.view(batch_size, num_images*t, vision_hidden_size)
        return reshaped_image_hidden_states

    # def forward(self, pixel_values):
    #     # return self.action_head(torch.zeros(pixel_values.shape[0], 128, device=self.device))
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            pixel_values = pixel_values[..., [-1], :, :, :].squeeze(dim=1)
            ...
            # image_hidden_states = self.seperate_batch_num_img(
            #     batch_size,
            #     num_images,
        ...
        # Take the last logit
        output = self.action_head(image_hidden_states[..., [-1], :])
...
class Idefics2ModelForRLFeatureExtraction(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is not None:
            batch_size, num_images, num_channels, height, width = pixel_values.shape
            pixel_values = pixel_values.to(dtype=self.dtype)  # fp16 compatibility
            pixel_values = pixel_values.reshape(batch_size * num_images, *pixel_values.shape[2:])

            # Remove padding images - padding images are full 0.
            # nb_values_per_image = pixel_values.shape[1:].numel()
            # real_images_inds = (pixel_values == 0.0).sum(dim=(-1, -2, -3)) != nb_values_per_image
            # pixel_values = pixel_values[real_images_inds].contiguous()
            # TODO: Look into this if have time, here I disabled removal of padding images
