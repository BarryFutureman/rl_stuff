...
for i in range(32):
    ...
    for img_index in range(1, len(render_stack)):
        blended_obs = blended_obs * np.array([0.5, 0.8, 1.2]) + render_stack[img_index]
    ...
    for j in range(num_frame_stack):
        ...
        if frame_num >= 0:
            ...
            game_context += f"<image>\nACTION::{act2action_string(action_at_frame)}\n\n"
            context_images.append(f"images/img_{frame_num}.png")
    current_frame_prompt = f"<image>\nACTION::"
    ...
    new_json_item = {"context": context_prompt,
