...
class MemEfficientRolloutBuffer(BaseBuffer):
    """
    ...
    def add(
        ...
        print(obs.shape, np.array(obs[:, [-1], ...]).shape)
        self.observations[self.pos] = np.array(obs[:, [-1], ...])
        self.actions[self.pos] = np.array(action)
        self.rewards[self.pos] = np.array(reward)
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        while start_idx < self.buffer_size * self.n_envs:
            yield self._get_samples_memory(indices, start_idx, batch_size)
            start_idx += batch_size

    def _get_samples_memory(
            self,
            indices,
            start_idx,
            batch_size,
        ...
        obs_lst = []
        memory_len = self.observation_space.shape[0]
        raise NotImplementedError()
        # It just does not work like this


        # for i in range(batch_size):
        #     if start_idx - i < memory_len-1:
        #         obs_lst.append(np.zeros(self.observations[batch_inds]))
        #     self.observations[batch_inds - :batch_inds]
        #     raise NotImplementedError

        combined_obs = np.concatenate(self.observations[batch_inds - self.observation_space.shape[0]:batch_inds],
                                      axis=1)
...
class DictRolloutBuffer(MemEfficientRolloutBuffer):
    """
    ...
    def __init__(
        ...
        super(MemEfficientRolloutBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        super(MemEfficientRolloutBuffer, self).reset()
