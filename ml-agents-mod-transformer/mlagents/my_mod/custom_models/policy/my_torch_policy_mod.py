...
from mlagents.my_mod.custom_models.policy.my_policy import Policy
...
class MyTorchPolicy(Policy):
    ...
    def evaluate(
        ...
        """
        obs = decision_requests.obs[1]
        masks = self._extract_masks(decision_requests)
        tensor_obs = [torch.as_tensor(np_ob) for np_ob in obs]
        memories = decision_requests.obs[0]  # torch.as_tensor(decision_requests.obs[0])
        ...
        if True:  # We want memory, as a tensor
            run_out["memory_out"] = memories  # ModelUtils.to_numpy(memories).squeeze(0)
        return run_out
    ...
    def get_action(
        ...
        # TODO: IMPORTANT! Currently the code only work for a single agent
        #  since I basically remove agent id check. In the future, if I ever need
        #  some parallel training, this code needs to change.
        # self.save_memories(global_agent_ids, run_out.get("memory_out"))
        # Save the context
        self.obs_context = run_out.get("memory_out")
