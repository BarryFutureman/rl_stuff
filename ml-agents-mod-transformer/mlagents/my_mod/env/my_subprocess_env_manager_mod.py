...
from mlagents.my_mod.env.my_env_manager import EnvManager, EnvironmentStep, AllStepResult
    ...
    def _preprocess_steps(self):
        for env_worker in self.env_workers:
            if not env_worker.waiting:
                prev_step = env_worker.previous_step
                for brain_name, step_tuple in prev_step.current_all_step_result.items():
                    print(self.policies[brain_name].obs_context)
                    if brain_name in self.policies:
                        print(step_tuple[0])
                        print(step_tuple[0].obs[0].size)
                        # Add context
                        step_tuple[0].obs = [self.policies[brain_name].obs_context, step_tuple[0].obs]
                        env_worker.previous_step.current_all_step_result[brain_name] = step_tuple
