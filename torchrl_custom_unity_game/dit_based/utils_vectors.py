...
def make_base_env(**kwargs):
    ...
    kwargs["side_channels"] = [channel]
    channel.set_configuration_parameters(time_scale=1, capture_frame_rate=-1, target_frame_rate=-1)

    # Create your Unity environment.
    # from mlagents_envs.registry import default_registry
    # unity_env = default_registry["Basic"].make(**kwargs, worker_id=random.randint(0, 100))

    unity_env = UnityEnvironment(  # file_name=None
        file_name="C:\\Files\\UnityProjects\\RoboticsGym2021\\Builds\\MobaGym\\RoboticsGym2021.exe",
    ...
    # env = UnityMLAgentsEnv(registered_name="Basic", categorical_actions=True)
...
def make_parallel_env(env_name, num_envs, device, env_kwargs=None, context_length=4, is_test=False):
    ...
    #     UnsqueezeTransform(dim=-2, in_keys=["observation"], out_keys=["observation"]))  # Add new dimension
    env.append_transform(CatFrames(N=context_length, dim=-2, in_keys=["observation"],
                                   out_keys=["observation"]))  # Adjust dim for CatFrames
...
def make_ppo_modules(proof_environment, device, model_dir=None, model_kwargs=None):
    ...
    value_transformer_model = model2.TransformerModule(
        in_features=int(input_shape[-1]),
        out_features=1,
        device=device,
    ...
    policy_module = torch.nn.Sequential(
        policy_transformer_model,
...
def save_ppo_models(actor, critic, optimizer, model_name, save_dir):
    ...
    actor_model = policy_module[0].module[0].model
